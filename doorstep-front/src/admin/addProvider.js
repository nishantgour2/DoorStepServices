...
import './addcat.css'
import {createProvider,getCategories} from './apiAdmin'
...
const AddProvider = () => {
    ...
    const clickSubmit = (event) => {
        ...
        createProvider(user._id,token,formData)
        .then(data => {
            if(data.error){
                setValues({...values,error:data.error})
            }
        ...
        {categories && categories.map((c,i) => (<option key={i} value={c._id}>{c.name}</option>))}
            ...
            <button className='btn btn-outline-primary'>Add Service Provider</button>
            </form>
